...
`);te=b.pop()??"";for(const ce of b){const E=ce.trim();E&&$e(E)}}te.trim()&&$e(te.trim());const Ee=Math.round((typeof performance<"u"?performance.now():Date.now())-U);if(K.error)throw B.warn(`api ← POST ${r} stream error (${Ee}ms)`,K.error.message),new be(K.error.message,K.error.status);if(!K.final)throw B.error(`api ← POST ${r} stream incomplete (${Ee}ms)`,{sawProgress:K.sawProgress}),new be(K.sawProgress?"Import stopped before finishing (server crash, out of memory, or gateway timeout). On TrueNAS, set memory limit to at least 1G, pull latest image, and recreate the app.":"Import failed to start on the server. Check container logs and that you are on the latest image.",500);return B.info(`api ← POST ${r} stream done (${Ee}ms)`),K.final}const q={ui:()=>j("/ui"),me:async()=>{var d;const r=await j("/me");return Lt(r.csrfToken||((d=r.user)==null?void 0:d.csrfToken)),r},login:async(r,d)=>{var C;const D=await j("/login",{method:"POST",body:JSON.stringify({username:r,password:d})});return Lt((C=D.user)==null?void 0:C.csrfToken),D},logout:async()=>{try{return await j("/logout",{method:"POST"})}finally{Lt("")}},calendars:()=>j("/calendars"),createCalendar:r=>j("/calendars",{method:"POST",body:JSON.stringify(r)}),holidayCountries:()=>j("/holidays/countries"),updateCalendar:(r,d)=>j(`/calendars/${r}`,{method:"PATCH",body:JSON.stringify(d)}),deleteCalendar:r=>j(`/calendars/${r}`,{method:"DELETE"}),calendarEvents:(r,d,D)=>{const C=new URLSearchParams({from:d,to:D}).toString();return j(`/calendars/${r}/events?${C}`)},getEvent:(r,d)=>j(`/calendars/${r}/events/${ve(d)}`),createEvent:(r,d)=>j(`/calendars/${r}/events`,{method:"POST",body:JSON.stringify(d)}),updateEvent:(r,d,D)=>j(`/calendars/${r}/events/${ve(d)}`,{method:"PATCH",body:JSON.stringify(D)}),deleteEvent:(r,d)=>j(`/calendars/${r}/events/${ve(d)}`,{method:"DELETE"}),exportCalendar:async r=>{const d=await fetch(`/api/calendars/${r}/export`,{credentials:"same-origin"});if(!d.ok){let F=`Export failed (${d.status})`;try{const S=await d.json();S.error&&(F=S.error)}catch{}throw new be(F,d.status)}const D=d.headers.get("Content-Disposition")||"",C=/filename="([^"]+)"/i.exec(D),x=(C==null?void 0:C[1])||`calendar-${r}.ics`;return{blob:await d.blob(),filename:x}},importCalendar:(r,d,D)=>ta(`/calendars/${r}/import`,d,"text/calendar; charset=utf-8",D),directory:()=>j("/directory"),shares:r=>j(`/calendars/${r}/shares`),share:(r,d,D)=>j(`/calendars/${r}/shares`,{method:"POST",body:JSON.stringify({username:d,access:D})}),revoke:(r,d)=>j(`/calendars/${r}/shares`,{method:"DELETE",body:JSON.stringify({href:d})}),addressbooks:()=>j("/addressbooks"),createAddressBook:r=>j("/addressbooks",{method:"POST",body:JSON.stringify(r)}),updateAddressBook:(r,d)=>j(`/addressbooks/${r}`,{method:"PATCH",body:JSON.stringify(d)}),deleteAddressBook:(r,d=!1)=>j(`/addressbooks/${r}`,{method:"DELETE",body:JSON.stringify({force:d})}),exportAddressBook:async r=>{const d=await fetch(`/api/addressbooks/${r}/export`,{credentials:"same-origin"});if(!d.ok){let F=`Export failed (${d.status})`;try{const S=await d.json();S.error&&(F=S.error)}catch{}throw new be(F,d.status)}const D=d.headers.get("Content-Disposition")||"",C=/filename="([^"]+)"/i.exec(D),x=(C==null?void 0:C[1])||`contacts-${r}.vcf`;return{blob:await d.blob(),filename:x}},importAddressBook:(r,d,D)=>ta(`/addressbooks/${r}/import`,d,"text/vcard; charset=utf-8",D),contacts:(r,d="")=>{const D=d.trim()?`?q=${encodeURIComponent(d.trim())}`:"";return j(`/addressbooks/${r}/contacts${D}`)},getContact:(r,d)=>j(`/addressbooks/${r}/contacts/${ve(d)}`),createContact:(r,d)=>j(`/addressbooks/${r}/contacts`,{method:"POST",body:JSON.stringify(d)}),updateContact:(r,d,D)=>j(`/addressbooks/${r}/contacts/${ve(d)}`,{method:"PATCH",body:JSON.stringify(D)}),deleteContact:(r,d)=>j(`/addressbooks/${r}/contacts/${ve(d)}`,{method:"DELETE"}),exportContact:async(r,d)=>{const D=await fetch(`/api/addressbooks/${r}/contacts/${ve(d)}/export`,{credentials:"same-origin"});if(!D.ok){let S=`Export failed (${D.status})`;try{const ne=await D.json();ne.error&&(S=ne.error)}catch{}throw new be(S,D.status)}const C=D.headers.get("Content-Disposition")||"",x=/filename="([^"]+)"/i.exec(C),U=(x==null?void 0:x[1])||"contact.vcf";return{blob:await D.blob(),filename:U}},contactPhotoUrl:(r,d)=>`/api/addressbooks/${r}/contacts/${ve(d)}/photo`,tasks:(r={})=>{const d=new URLSearchParams;r.q&&d.set("q",r.q),r.sort&&d.set("sort",r.sort),r.order&&d.set("order",r.order);const D=d.toString()?`?${d}`:"";return j(`/tasks${D}`)},createTask:r=>j("/tasks",{method:"POST",body:JSON.stringify(r)}),updateTask:(r,d,D)=>j(`/tasks/${r}/${ve(d)}`,{method:"PATCH",body:JSON.stringify(D)}),deleteTask:(r,d)=>j(`/tasks/${r}/${ve(d)}`,{method:"DELETE"}),bulkTasks:r=>j("/tasks/bulk",{method:"POST",body:JSON.stringify(r)}),notes:(r={})=>{const d=new URLSearchParams;r.q&&d.set("q",r.q),r.sort&&d.set("sort",r.sort),r.order&&d.set("order",r.order);const D=d.toString()?`?${d}`:"";return j(`/notes${D}`)},createNote:r=>j("/notes",{method:"POST",body:JSON.stringify(r)}),updateNote:(r,d,D)=>j(`/notes/${r}/${ve(d)}`,{method:"PATCH",body:JSON.stringify(D)}),deleteNote:(r,d)=>j(`/notes/${r}/${ve(d)}`,{method:"DELETE"})},nn="0.11.1-fork.4",sn="https://github.com/offsyanka99/Baikal/tree/master/docs";function c(r){return r.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function qt(r){return r==="readwrite"?'<span class="badge badge-admin">full access</span>':r==="read"?'<span class="badge">read-only</span>':r==="owner"?'<span class="badge badge-ok">owner</span>':`<span class="badge">${c(r)}</span>`}function wt(r){const d=[`${r.imported} new`,`${r.updated} updated`];return r.skipped>0&&d.push(`${r.skipped} skipped`),d.join(", ")}const rn={"my-calendars":{title:"Calendar",paragraphs:["Create and edit calendars, then share them with other Baïkal users.","CalDAV clients (Thunderbird, Apple Calendar, DAVx⁵, Home Assistant, …) keep using /dav.php/ — this portal is for management only."]},owned:{title:"Owned",paragraphs:["Calendars you own appear here. Select one to edit details, import/export, or share.","Badges show ownership, read-only mode, and holiday calendars."]},"add-calendar":{title:"Add calendar",paragraphs:["Create a normal calendar, or a holidays calendar for a chosen country (public holidays for this year and next are imported automatically via Nager.Date).","Import .ics creates a new calendar (using the display name, or the file name if the name is empty) and imports all events into it. Large files show a progress dialog.","Read-only (for everyone) blocks import in the portal, forces shares to read-only, and rejects CalDAV writes (PUT/DELETE/…) from clients such as DAVx⁵, Thunderbird, and Home Assistant."]},"shared-with-me":{title:"Shared with me",paragraphs:["Calendars other users shared with you. You can export a copy; name, color, and sharing are managed by the owner."]},"calendar-details":{title:"Calendar details",paragraphs:["Display name, color, and description are stored on the calendar and are visible to CalDAV clients.","The URI is the internal calendar path used by CalDAV; it does not change when you rename the display name."]},"import-export":{title:"Import / export",paragraphs:["Export downloads a standard .ics file of the whole calendar.","Import merges VEVENT, VTODO, and VJOURNAL components. The same UID updates an existing object; new UIDs create objects.","Large imports show a progress dialog (read → upload → server import) with elapsed time; keep the tab open until it finishes.","Read-only calendars can still be exported, but import is disabled so reference data (e.g. holidays) stays intact."]},share:{title:"Share",paragraphs:["Share this calendar with another Baïkal user. Choose read-only or full access.","This is the same sharing model as the classic /dav.php/ browser, without typing mailto: addresses.","If the calendar is marked read-only, shares are always read-only for everyone."]},"my-contacts":{title:"Contacts",paragraphs:["Manage address books and individual contacts for CardDAV. Clients (Thunderbird, DAVx⁵, …) keep using /dav.php/.","Create or rename address books, search contacts, add/edit/delete cards, upload photos, and import/export .vcf files."]},tasks:{title:"Tasks",paragraphs:["Tasks are CalDAV VTODO items stored in your calendars. They sync with Apple Reminders, Thunderbird, DAVx⁵, and other clients via /dav.php/.","Subtasks use RELATED-TO;RELTYPE=PARENT (same calendar). Add a subtask from a parent, or set Parent in the form. Deleting a parent promotes its children to top-level.","Click a column header to sort. Create tasks on any writable calendar that allows VTODO components."]},notes:{title:"Notes",paragraphs:["Notes are CalDAV VJOURNAL items stored in your calendars. Compatible clients sync them over /dav.php/.","Click a column header to sort. Pick a writable calendar when creating a note."]},"address-books":{title:"Address books",paragraphs:["Address books you own. Select one to manage its contacts.","You can create, rename, or delete address books here. Deleting a non-empty book requires confirmation."]},contacts:{title:"Contacts",paragraphs:["Search filters by name, email, phone, org, notes, and custom fields.","Add or select a contact to edit fields. Multiple emails and phones are supported.","Photos are resized to 256px JPEG and stored in the vCard so CardDAV clients can sync them.","Custom fields support any language in the label and value (including Cyrillic). They are stored as X-BAIKAL-CUSTOM in the vCard so non-English labels work; CardDAV clients that ignore unknown properties will not show them."]},"contact-import-export":{title:"Import / export contacts",paragraphs:["Export downloads a multi-vCard .vcf file of every contact in the address book.","Import accepts standard .vcf files (Thunderbird, Apple Contacts, Google). Same UID updates an existing card; new UIDs create cards.","Large imports show a progress dialog with elapsed time — keep the tab open until the result appears."]}};function Ce(r,d,D="h2"){const C=D;return`<div class="section-title-row">
    ...
    <div class="info-modal" id="info-modal" hidden role="dialog" aria-modal="true" aria-labelledby="info-modal-title">
    ...
    </div>`}function ln(r){let d=null,D=null,C="calendars",x=[],U=[],F=[],S=null,ne=[],W=!1,ie=!1,te=null,K=null,$e={y:new Date().getFullYear(),m:new Date().getMonth()},Ee=[],Y=!1,_=!1,b=null,ce=!1,E=null,ut="",Ze=null,de=[],I=null,xe=[],Be="",G=null,k=null,Z=!1,oe=!1,ye=!1,ue=null,he=null,we=!1,p=!1,ke=null,Le=null,M=null,mt=null,Mt=!1,ze={timeFormat:"auto",weekStart:"auto",logLevel:"off"},Ie=null;function St(e){if(!e)return;const t=(e.timeFormat||"auto").toLowerCase(),a=(e.weekStart||"auto").toLowerCase();ze={timeFormat:t==="12h"||t==="24h"?t:"auto",weekStart:a==="monday"||a==="sunday"?a:"auto",logLevel:e.logLevel||"off"},tn(ze.logLevel)}let me=[],et=[],_e=[],Ve=[],pt="",ft="",qe="due",Oe="asc",Ke="dtstart",He="desc",pe=null,Ne=null,P=null,z=null,J=!1,le=!1,se=[];function v(e,t){D={type:e,message:t}}function O(){D=null}async function sa(){B.event("bootstrap.start");try{const e=await q.ui();St(e.ui)}catch(e){B.debug("bootstrap: /api/ui failed",e instanceof Error?e.message:e)}try{const e=await q.me();d=e.user,St(e.ui),B.event("bootstrap.session",{username:(d==null?void 0:d.username)??null}),await ge()}catch(e){e instanceof be&&e.status===401?(d=null,B.event("bootstrap.anonymous")):(B.error("bootstrap failed",e instanceof Error?e.message:e),v("error",e instanceof Error?e.message:"Failed to load"))}m()}async function ge(){B.debug("loadHome");const[e,t,a]=await Promise.all([q.calendars(),q.directory().catch(()=>({users:[]})),q.addressbooks()]);if(x=e.calendars,U=t.users,de=a.addressbooks,B.event("loadHome",{calendars:x.length,addressBooks:de.length,directory:U.length}),F.length===0)try{F=(await q.holidayCountries()).countries}catch{F=[]}if(S!==null&&!x.some(o=>o.id===S)&&(S=null,ne=[],W=!1,te=null),S===null){const o=Ft();o&&(S=o.id)}S!==null&&W?await tt(S):S!==null&&(ne=[]),C==="calendars"&&await Se(),I!==null&&!de.some(o=>o.id===I)&&(I=null,xe=[],G=null,k=null,Z=!1),K!==null&&!de.some(o=>o.id===K)&&(K=null),I===null&&de.length>0&&(I=de[0].id),I!==null&&C==="contacts"&&await Fe(I)}async function tt(e){ne=(await q.shares(e)).shares}function Ft(){const e=x.filter(a=>a.canShare);if(e.length===0)return null;const t=a=>{const o=a.uri.toLowerCase(),l=a.displayname.toLowerCase();return o==="default"||l==="default"||l==="default calendar"};return e.find(t)??e[0]??null}function ee(e){const t=e.getFullYear(),a=String(e.getMonth()+1).padStart(2,"0"),o=String(e.getDate()).padStart(2,"0");return`${t}-${a}-${o}`}function ra(e,t){const a=new Date(e,t,1),o=new Date(e,t+1,0);return{from:ee(a),to:ee(o)}}function Dt(e){if(/^\d{4}-\d{2}-\d{2}$/.test(e)){const[a,o,l]=e.split("-").map(Number);return new Date(a,o-1,l)}const t=new Date(e);if(Number.isNaN(t.getTime())){const[a,o,l]=e.slice(0,10).split("-").map(Number);return new Date(a,(o||1)-1,l||1)}return new Date(t.getFullYear(),t.getMonth(),t.getDate())}function oa(e){const t=Dt(e.start);if(!e.end)return[ee(t)];let a=Dt(e.end);if(!e.allDay&&!/^\d{4}-\d{2}-\d{2}$/.test(e.end)){const s=new Date(e.end);!Number.isNaN(s.getTime())&&s.getHours()===0&&s.getMinutes()===0&&s.getSeconds()===0&&s.getTime()>new Date(e.start).getTime()&&(a=new Date(a.getFullYear(),a.getMonth(),a.getDate()-1))}if(a<t)return[ee(t)];const o=[],l=new Date(t.getFullYear(),t.getMonth(),t.getDate()),u=new Date(a.getFullYear(),a.getMonth(),a.getDate());let n=0;for(;l<=u&&n++<370;)o.push(ee(l)),l.setDate(l.getDate()+1);return o.length?o:[ee(t)]}function Ct(e,t){const a=e.slice(0,10),o=(t||a).slice(0,10);if(a===o){const A=nt(a);return{start:A.start,end:A.end}}const[l,u,n]=a.split("-").map(Number),[s,i,f]=o.split("-").map(Number),y=Me(new Date(l,u-1,n,9,0,0,0)),g=Me(new Date(s,i-1,f,17,0,0,0));return{start:y,end:g}}function la(e,t){const a=We(e);let o=t?We(t):a;if(t&&!/^\d{4}-\d{2}-\d{2}$/.test(t)){const l=new Date(t);if(!Number.isNaN(l.getTime())&&l.getHours()===0&&l.getMinutes()===0&&l.getTime()>new Date(e).getTime()){const u=Dt(t);u.setDate(u.getDate()-1),o=ee(u)}}return{start:a,end:o}}async function Se(){if(S===null){Ee=[];return}const{from:e,to:t}=ra($e.y,$e.m);Y=!0,B.debug("loadMonthEvents",{selectedId:S,from:e,to:t});try{Ee=(await q.calendarEvents(S,e,t)).events,B.event("monthEvents.loaded",{calendarId:S,count:Ee.length,from:e,to:t})}catch(a){Ee=[],B.warn("loadMonthEvents failed",a instanceof Error?a.message:a)}finally{Y=!1}}function ia(e,t){return new Date(e,t,1).toLocaleString(void 0,{month:"long",year:"numeric"})}function ca(e){const t=e.summary||"(No title)";if(e.allDay||/^\d{4}-\d{2}-\d{2}$/.test(e.start))return t;const a=new Date(e.start);return Number.isNaN(a.getTime())?t:`${a.toLocaleTimeString(void 0,Et())} ${t}`}function da(){const e=S!==null?x.find(h=>h.id===S):null,t=(e==null?void 0:e.displayname)??"Calendar",a=e!=null&&e.color?e.color.length>=7?e.color.slice(0,7):e.color:"#3B82F6",o=$e.y,l=$e.m,u=new Date(o,l,1),n=Nt(),s=(u.getDay()-n+7)%7,i=new Date(o,l+1,0).getDate(),f=new Date(o,l,0).getDate(),g=ee(new Date),A=Pt(),T=new Map;for(const h of Ee)for(const R of oa(h)){const N=T.get(R)??[];N.push(h),T.set(R,N)}const w=[],L=Math.ceil((s+i)/7)*7;for(let h=0;h<L;h++){let R,N=!0,V;h<s?(R=f-s+h+1,N=!1,V=new Date(o,l-1,R)):h>=s+i?(R=h-(s+i)+1,N=!1,V=new Date(o,l+1,R)):(R=h-s+1,V=new Date(o,l,R));const H=ee(V),Q=H===g,fe=N?T.get(H)??[]:[],lt=Ze===H?50:3,Qe=fe.slice(0,lt),ht=fe.length-Qe.length,Ae=Qe.map(it=>{const Ot=S??0,vt=ca(it);return`<button type="button" class="month-event${it.allDay?"":" is-timed"}" title="${c(vt)}" style="--ev-color:${c(a)}"
            data-action="open-event" data-instance="${Ot}" data-uri="${c(it.uri)}" ${p?"disabled":""}>${c(vt)}</button>`}).join(""),xt=ht>0?`<button type="button" class="month-event-more" data-action="open-event-day" data-day="${c(H)}" title="Show all events this day" ${p?"disabled":""}>+${ht} more</button>`:"",It=!N&&(R===1||h===s+i)?V.toLocaleString(void 0,{month:"short",day:"numeric"}):String(R),gt=!!(e&&!e.readOnly&&(e.canShare||e.access==="readwrite"));w.push(`<div class="month-cell${N?"":" is-outside"}${Q?" is-today":""}${gt?" is-clickable":""}"${gt?` data-action="new-event-day" data-day="${c(H)}" role="button" tabindex="0" title="Add event on ${c(H)}"`:""}>
        <div class="month-daynum${Q?" is-today-num":""}">${c(It)}</div>
        <div class="month-events">${Ae}${xt}</div>
      </div>`)}const ae=e?Y?'<p class="muted small month-empty-hint">Loading events…</p>':"":'<p class="muted small month-empty-hint">No calendars yet — create one on the left. The grid stays empty until events exist.</p>';return`<section class="card month-cal-card">
      ...
      ${f&&!s?fa(t,l,u,i):""}
    </div>`}function Tt(e){var t;return e==="start"?String((b==null?void 0:b.start)||""):e==="end"?String((b==null?void 0:b.end)||""):e==="until"?((t=b==null?void 0:b.repeat)==null?void 0:t.until)||We(b==null?void 0:b.start)||ee(new Date):e==="due"?Ge(P==null?void 0:P.due):e==="dtstart"?Ge(z==null?void 0:z.dtstart):e==="bulk-due"?ut:e==="birthday"?String((k==null?void 0:k.birthday)||""):""}function De(e,t){if(e==="start"&&b){b={...b,start:t||""};return}if(e==="end"&&b){b={...b,end:t};return}if(e==="until"&&b){b={...b,repeat:{...b.repeat??bt(),until:t,endMode:"until"}};return}if(e==="due"&&P){if(t===null||t==="")P={...P,due:null};else if(/^\d{4}-\d{2}-\d{2}$/.test(t))P={...P,due:new Date(t+"T00:00:00").toISOString()};else{const a=new Date((t.length===16,t));P={...P,due:Number.isNaN(a.getTime())?t:a.toISOString()}}return}if(e==="dtstart"&&z){if(t===null||t==="")z={...z,dtstart:null};else if(/^\d{4}-\d{2}-\d{2}$/.test(t))z={...z,dtstart:new Date(t+"T00:00:00").toISOString()};else{const a=new Date((t.length===16,t));z={...z,dtstart:Number.isNaN(a.getTime())?t:a.toISOString()}}return}if(e==="birthday"&&k){k={...k,birthday:t&&/^\d{4}-\d{2}-\d{2}/.test(t)?t.slice(0,10):null};return}e==="bulk-due"&&(ut=t||"")}function fa(e,t,a,o){const l=at(t),u=(E==null?void 0:E.viewY)??Number(l.date.slice(0,4)),n=(E==null?void 0:E.viewM)??Number(l.date.slice(5,7))-1,s=Nt(),i=Pt(),y=(new Date(u,n,1).getDay()-s+7)%7,g=new Date(u,n+1,0).getDate(),A=new Date(u,n,0).getDate(),T=l.date,w=l.hm,L=new Date(u,n,1).toLocaleString(void 0,{month:"long",year:"numeric"}),ae=[],h=Math.ceil((y+g)/7)*7;for(let N=0;N<h;N++){let V,H,Q=!1;N<y?(V=A-y+N+1,H=new Date(u,n-1,V),Q=!0):N>=y+g?(V=N-(y+g)+1,H=new Date(u,n+1,V),Q=!0):(V=N-y+1,H=new Date(u,n,V));const fe=ee(H),lt=fe===T,Qe=fe===ee(new Date);ae.push(`<button type="button" class="dt-day${Q?" is-outside":""}${lt?" is-selected":""}${Qe?" is-today":""}" data-action="dt-pick-day" data-dt-field="${e}" data-day="${c(fe)}">${V}</button>`)}const R=a?"":`<div class="dt-times" role="listbox" aria-label="Time">
          ${pa().map(N=>{const V=(()=>{const[H,Q]=N.split(":").map(Number);return new Date(2e3,0,1,H,Q).toLocaleTimeString(void 0,Et())})();return`<button type="button" class="dt-time${N===w?" is-selected":""}" data-action="dt-pick-time" data-dt-field="${e}" data-hm="${N}" role="option" aria-selected="${N===w}">${c(V)}</button>`}).join("")}
          ...
          <form class="stack" data-form="edit-event">
            ...
            <fieldset class="event-repeat" ${u?"disabled":""}>
              ...
              ${a?`<div class="form-grid form-grid-2" style="margin-top:0.5rem">
                      ...
                      ${y==="until"?Ye({field:"until",name:"repeatUntil",label:"Until",value:A,dateOnly:!0,disabled:u,allowClear:!0}):y==="count"?`<label>Occurrences
    ...
    </div>`}function ga(e,t){const a=x.find(o=>o.id===t);return{uri:"",instanceId:t,calendarId:(a==null?void 0:a.calendarId)??0,calendarName:(a==null?void 0:a.displayname)??"Calendar",calendarUri:(a==null?void 0:a.uri)??"",uid:"",summary:"",description:"",location:"",start:e,end:e,allDay:!0,hasRrule:!1,repeat:bt(),readOnly:!1,canWrite:!0}}async function Fe(e){xe=(await q.contacts(e,Be)).contacts,G!==null&&!xe.some(a=>a.uri===G)&&(G=null,Z||(k=null,ue=null,he=null,we=!1))}async function Je(){const e=await q.tasks({q:pt,sort:qe,order:Oe});me=e.tasks,_e=e.calendars;const t=new Set(me.map(a=>X(a.instanceId,a.uri)));se=se.filter(a=>t.has(a)),pe!==null&&!me.some(a=>`${a.instanceId}|${a.uri}`===pe)&&(pe=null,J||(P=null))}async function st(){const e=await q.notes({q:ft,sort:Ke,order:He});et=e.notes,Ve=e.calendars,Ne!==null&&!et.some(t=>`${t.instanceId}|${t.uri}`===Ne)&&(Ne=null,le||(z=null))}function X(e,t){return`${e}|${t}`}function Ut(e){if(!e)return"—";try{const t=new Date(e);return Number.isNaN(t.getTime())?e:t.toLocaleString(void 0,{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return e}}function Ge(e){if(!e)return"";try{const t=new Date(e);if(Number.isNaN(t.getTime()))return"";const a=o=>String(o).padStart(2,"0");return`${t.getFullYear()}-${a(t.getMonth()+1)}-${a(t.getDate())}T${a(t.getHours())}:${a(t.getMinutes())}`}catch{return""}}function Pe(e,t,a,o,l,u=""){const n=a===t,s=n?o==="asc"?" ▲":" ▼":"";return`<th class="${`sortable-th${n?" is-sorted":""}${u?" "+u:""}`}" data-action="sort-${l}" data-sort="${c(t)}" role="columnheader" tabindex="0">${c(e)}${s}</th>`}async function va(e){if(I===null)return;const t=await q.getContact(I,e);G=e,Z=!1;const a=t.contact;k={...a,emails:Array.isArray(a.emails)?a.emails:[],phones:Array.isArray(a.phones)?a.phones:[],custom:Array.isArray(a.custom)?a.custom:[],address:a.address??jt(),birthday:a.birthday??null},ue=a.photoDataUri??(a.hasPhoto&&I!==null?`${q.contactPhotoUrl(I,e)}?t=${Date.now()}`:null),he=null,we=!1,oe=!0}function $a(){Z=!0,G=null,oe=!0,k={uri:"",displayname:"",firstname:"",lastname:"",fullname:"",org:"",title:"",emails:[""],phones:[{type:"cell",value:""}],address:{street:"",city:"",region:"",postal:"",country:""},birthday:null,url:"",note:"",custom:[],hasPhoto:!1,photoDataUri:null},ue=null,he=null,we=!1}function jt(){return{street:"",city:"",region:"",postal:"",country:""}}function wa(e){return new Promise((t,a)=>{const o=new FileReader;o.onload=()=>{const l=String(o.result??""),u=l.indexOf(",");t(u>=0?l.slice(u+1):l)},o.onerror=()=>a(new Error("Failed to read photo file")),o.readAsDataURL(e)})}function Bt(e,t={}){const a=`
      ...
      <span>Baïkal User Portal</span>`,o=d?`<nav class="topnav">
          ...
          <a class="brand" href="/portal/">${a}</a>
        </nav>`,u=!(W||ie||te!==null||K!==null||_||oe||ye)?Re():"",n=`
      ...
      ${n}
      ${on()}
      ${ka()}`}function Re(){return D?`<div class="flash flash-${c(D.type)}" role="status">
      ...
      ${f}
      ${g}
      ${ha()}`,w=de.map($=>`<div class="cal-row${$.id===I?" is-selected":""}" data-action="select-ab" data-id="${$.id}" role="button" tabindex="0">
          <span class="cal-swatch cal-swatch-empty"></span>
          <span class="cal-row-text">
            <span class="cal-row-title">${c($.displayname)}</span>
          ...
          <span class="cal-row-actions">
            ...
            <button type="button" class="btn btn-small btn-danger" data-action="delete-ab" data-id="${$.id}" ${p?"disabled":""}>Delete</button>
              ...
              </tr>`}).join(""),h=k,R=Array.isArray(h==null?void 0:h.emails)&&h.emails.length>0?h.emails:[""],N=Array.isArray(h==null?void 0:h.phones)&&h.phones.length>0?h.phones:[{type:"cell",value:""}],V=(h==null?void 0:h.address)??jt(),H=R.map(($,re)=>`<div class="multi-row" data-multi="email" data-idx="${re}">
        ...
        </div>`).join(""),Q=N.map(($,re)=>`<div class="multi-row multi-row-phone" data-multi="phone" data-idx="${re}">
          ...
          <button type="button" class="btn btn-ghost btn-small" data-action="remove-phone" data-idx="${re}" ${N.length<=1?"disabled":""} title="Remove">×</button>
        </div>`).join(""),fe=Array.isArray(h==null?void 0:h.custom)?h.custom:[],lt=fe.length===0?'<p class="muted small" style="margin:0 0 0.5rem">No custom fields yet. Labels and values can use any language (e.g. Супруг, 日本語).</p>':fe.map(($,re)=>`<div class="multi-row multi-row-custom" data-multi="custom" data-idx="${re}">
                ...
                <form class="stack" data-form="contact">
                  <div class="contact-photo-row">
                    <div class="contact-photo-preview">
                      ${ue?`<img src="${c(ue)}" alt="Contact photo" />`:`<span class="contact-avatar contact-avatar-fallback contact-avatar-lg" aria-hidden="true">${c((h.fullname||h.firstname||"?").slice(0,1).toUpperCase())}</span>`}
                    </div>
                    <div class="stack stack-tight" style="flex:1">
                      ...
                      ${ue||h.hasPhoto?`<button type="button" class="btn btn-ghost btn-small" data-action="remove-photo" ${p?"disabled":""}>Remove photo</button>`:""}
                  ...
                  <div class="form-actions-row form-actions-wrap">
                    ...
                    ${!Z&&h.uri?`<button type="button" class="btn" data-action="export-contact" ${p?"disabled":""}>Export .vcf</button>`:""}
          ...
          </div>`:"",ht=ye&&L?`<div class="cal-modal" id="ab-edit-modal" role="dialog" aria-modal="true" aria-labelledby="ab-modal-title">
          ...
          </div>`:"",Ae=K!==null?de.find($=>$.id===K)??null:null,xt=Ae?`<div class="cal-modal" id="ab-delete-modal" role="dialog" aria-modal="true" aria-labelledby="ab-delete-title">
      ...
      ${xt}
      ${ht}
      ${Qe}`,gt=C==="calendars"?"my-calendars":C==="contacts"?"my-contacts":C==="tasks"?"tasks":"notes",it=Na(),Ot=Ta(),vt=C==="calendars"?T:C==="contacts"?It:C==="tasks"?it:Ot;r.innerHTML=Bt(`
      ...
      ${vt}
    `),document.body.classList.toggle("cal-modal-open",W||ie||te!==null||K!==null||_||oe||ye||M!==null),document.body.classList.toggle("layout-contacts",C==="contacts"),document.body.classList.toggle("layout-calendars",C==="calendars"),document.body.classList.toggle("layout-tasks",C==="tasks"||C==="notes")}function Da(e){const t=new Map;for(const f of e)f.uid&&t.set(f.uid,f);const a=new Map(e.map((f,y)=>[X(f.instanceId,f.uri),y])),o=new Map,l=[];for(const f of e){const y=f.parentUid;if(y&&t.has(y)&&y!==f.uid){const g=o.get(y)??[];g.push(f),o.set(y,g)}else l.push(f)}const u=(f,y)=>(a.get(X(f.instanceId,f.uri))??0)-(a.get(X(y.instanceId,y.uri))??0);l.sort(u);for(const[,f]of o)f.sort(u);const n=[],s=new Set,i=(f,y)=>{const g=f.uid||X(f.instanceId,f.uri);if(!s.has(g)){s.add(g),n.push({task:f,depth:Math.min(y,8)});for(const A of o.get(f.uid)??[])i(A,y+1);s.delete(g)}};for(const f of l)i(f,0);for(const f of e)n.some(y=>y.task===f)||n.push({task:f,depth:0});return n}function Ca(e){const t=new Set([e]);if(!e)return t;let a=!0;for(;a;){a=!1;for(const o of me)o.parentUid&&t.has(o.parentUid)&&o.uid&&!t.has(o.uid)&&(t.add(o.uid),a=!0)}return t}function Ea(e,t){const a=e.instanceId,o=t||!e.uid?new Set:Ca(e.uid),l=me.filter(s=>s.uid&&s.instanceId===a&&!o.has(s.uid)&&s.uid!==e.uid),u=e.parentUid||"",n=['<option value="">None (top-level)</option>',...l.map(s=>`<option value="${c(s.uid)}" ${s.uid===u?"selected":""}>${c(s.summary||s.uid)}</option>`)];if(u&&!l.some(s=>s.uid===u)){const s=me.find(i=>i.uid===u);n.push(`<option value="${c(u)}" selected>${c((s==null?void 0:s.summary)||u)} (current)</option>`)}return n.join("")}function Kt(){const e=new Set(se);return me.filter(t=>e.has(X(t.instanceId,t.uri))&&t.canWrite&&!t.readOnly)}function Na(){const e=w=>({"NEEDS-ACTION":"To do","IN-PROCESS":"In progress",COMPLETED:"Done",CANCELLED:"Cancelled"})[w]||w,t=Da(me),a=me.filter(w=>w.canWrite&&!w.readOnly).map(w=>X(w.instanceId,w.uri)),o=a.length>0&&a.every(w=>se.includes(w)),l=se.length>0,n=Kt().length,s=me.length===0?`<tr class="contacts-empty-row"><td colspan="6" class="muted">${pt?"No tasks match your search.":"No tasks yet. Add one below."}</td></tr>`:t.map(({task:w,depth:L})=>{const ae=X(w.instanceId,w.uri),h=!J&&ae===pe?" is-selected":"",R=se.includes(ae),N=w.status==="COMPLETED"?"badge-ok":w.status==="CANCELLED"?"":"badge-admin",V=L>0?` style="--task-depth:${L}"`:"",H=L>0?'<span class="task-subtask-marker" aria-hidden="true">↳</span>':"",Q=w.canWrite&&!w.readOnly;return`<tr class="contact-table-row task-row${L>0?" is-subtask":""}${h}${R?" is-checked":""}" data-action="select-task" data-instance="${w.instanceId}" data-uri="${c(w.uri)}" tabindex="0" role="button"${V}>
      ...
      <path d="M3.5 8.5 6.5 11.5 12.5 4.5" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        title="${c(L)}" aria-label="${c(L)}" ${p||n===0?"disabled":""}>${i}</button>`,y=l?`<div class="bulk-bar" style="margin-top:0.75rem">
            <div class="bulk-bar-row">
              ...
              <div class="bulk-group bulk-group-due">
                ${Ye({field:"bulk-due",name:"bulkDue",label:"Due",value:ut,dateOnly:!1,disabled:p||n===0,allowClear:!0})}
          ...
          </div>`:"",g=P,A=_e.map(w=>`<option value="${w.id}" ${g&&g.instanceId===w.id?"selected":""}>${c(w.displayname)}</option>`).join(""),T=g?`<div class="card">
      ...
      <section class="card contacts-main-card items-list-card">
        ${Ce("Tasks","tasks")}
        <div class="contact-toolbar" style="margin-top:0.75rem">
          <input type="search" data-action="task-search" placeholder="Search tasks…" value="${c(pt)}" aria-label="Search tasks" ${p?"disabled":""} />
    ...
    </div>`}function Ta(){const e=et.length===0?`<tr class="contacts-empty-row"><td colspan="3" class="muted">${ft?"No notes match your search.":"No notes yet. Add one below."}</td></tr>`:et.map(l=>{const u=X(l.instanceId,l.uri),n=!le&&u===Ne?" is-selected":"",s=(l.description||"").replace(/\s+/g," ").slice(0,80);return`<tr class="contact-table-row${n}" data-action="select-note" data-instance="${l.instanceId}" data-uri="${c(l.uri)}" tabindex="0" role="button">
      ...
      <section class="card contacts-main-card items-list-card">
        ${Ce("Notes","notes")}
        <div class="contact-toolbar" style="margin-top:0.75rem">
          <input type="search" data-action="note-search" placeholder="Search notes…" value="${c(ft)}" aria-label="Search notes" ${p?"disabled":""} />
    ...
    </div>`}function xa(){const e=r.querySelector(".contacts-table-wrap"),t=r.querySelector(".contacts-ab-list"),a=r.querySelector(".calendars-owned-list");return{windowX:window.scrollX,windowY:window.scrollY,tableTop:(e==null?void 0:e.scrollTop)??null,abListTop:(t==null?void 0:t.scrollTop)??null,calListTop:(a==null?void 0:a.scrollTop)??null}}function Ia(e){requestAnimationFrame(()=>{requestAnimationFrame(()=>{if(window.scrollTo(e.windowX,e.windowY),e.tableTop!==null){const t=r.querySelector(".contacts-table-wrap");t&&(t.scrollTop=e.tableTop)}if(e.abListTop!==null){const t=r.querySelector(".contacts-ab-list");t&&(t.scrollTop=e.abListTop)}if(e.calListTop!==null){const t=r.querySelector(".calendars-owned-list");t&&(t.scrollTop=e.calListTop)}})})}function m(){const e=xa();d?Sa():zt(),Oa(),Ia(e),requestAnimationFrame(()=>{var t;ba(),(t=r.querySelector(".dt-time.is-selected"))==null||t.scrollIntoView({block:"center"})})}function Gt(e){const t=e.querySelector('input[name="color_picker"]'),a=e.querySelector('input[name="color"]');!t||!a||(t.addEventListener("input",()=>{a.value=t.value.toUpperCase()}),a.addEventListener("change",()=>{let o=a.value.trim();o&&!o.startsWith("#")&&(o=`#${o}`),/^#[0-9A-Fa-f]{6}/.test(o)&&(t.value=o.slice(0,7),a.value=o.toUpperCase())}))}function Oa(){r.querySelectorAll("[data-action]").forEach(h=>{h.addEventListener("click",R=>{const N=R.target.closest("[data-action]");((N==null?void 0:N.dataset.action)==="info"||(N==null?void 0:N.dataset.action)==="info-close")&&(R.preventDefault(),R.stopPropagation()),_a(R)})}),r.querySelectorAll("tr.contact-table-row[data-action], .cal-row[data-action], .month-cell[data-action]").forEach(h=>{h.addEventListener("keydown",R=>{(R.key==="Enter"||R.key===" ")&&(R.preventDefault(),h.click())})});const e=r.querySelector("#delete-cal-confirm"),t=r.querySelector("#delete-cal-submit");e==null||e.addEventListener("change",()=>{t&&(t.disabled=!e.checked||p)});const a=r.querySelector("#delete-ab-confirm"),o=r.querySelector("#delete-ab-submit");a==null||a.addEventListener("change",()=>{o&&(o.disabled=!a.checked||p)}),r.querySelectorAll("img.contact-avatar[data-avatar-fallback]").forEach(h=>{h.addEventListener("error",()=>{const R=h.dataset.avatarFallback||"?",N=document.createElement("span");N.className="contact-avatar contact-avatar-fallback",N.setAttribute("aria-hidden","true"),N.textContent=R,h.replaceWith(N)})}),Mt||(document.addEventListener("keydown",h=>{if(h.key==="Escape"){if(M&&(M.phase==="done"||M.phase==="error")){Ht();return}M||Xt()}}),Mt=!0);const l=r.querySelector('[data-form="login"]');l==null||l.addEventListener("submit",h=>{h.preventDefault(),Pa(l)});const u=r.querySelector('[data-form="share"]');u==null||u.addEventListener("submit",h=>{h.preventDefault(),Ra(u)});const n=r.querySelector('[data-form="edit-cal"]');n&&(Gt(n),n.addEventListener("submit",h=>{h.preventDefault(),ja(n)}));const s=r.querySelector('[data-form="edit-event"]');s==null||s.addEventListener("submit",h=>{h.preventDefault(),Ua(s)}),r.querySelectorAll('select[data-action="event-repeat-freq"], select[data-action="event-repeat-end"]').forEach(h=>{h.addEventListener("change",()=>{if(!b)return;const R=r.querySelector('[data-form="edit-event"]');if(!R)return;const N=new FormData(R),V=R.querySelector('input[name="allDay"]'),H=ot(N);H.endMode==="until"&&!H.until&&(H.until=We(String(N.get("start")??b.start??""))||ee(new Date)),b={...b,summary:String(N.get("summary")??b.summary),description:String(N.get("description")??b.description),location:String(N.get("location")??b.location),instanceId:Number(N.get("instanceId"))||b.instanceId,allDay:(V==null?void 0:V.checked)??b.allDay,start:String(N.get("start")??b.start??""),end:String(N.get("end")??b.end??"")||null,repeat:H,hasRrule:!!String(N.get("repeatFreq")??"").trim()},H.freq&&H.endMode==="until"&&(E==null?void 0:E.field)==="end"&&(E=null),m(),H.endMode==="until"&&requestAnimationFrame(()=>{var fe;const Q=r.querySelector('input[name="repeatUntil"]');Q==null||Q.focus();try{(fe=Q==null?void 0:Q.showPicker)==null||fe.call(Q)}catch{}})})});const i=r.querySelector('[data-form="create-cal"]');i&&(Gt(i),i.addEventListener("submit",h=>{h.preventDefault(),Ba(i)}));const f=r.querySelector('[data-form="create-ab"]');f==null||f.addEventListener("submit",h=>{h.preventDefault(),Ja(f)});const y=r.querySelector('[data-form="edit-ab"]');y==null||y.addEventListener("submit",h=>{h.preventDefault(),za(y)});const g=r.querySelector('[data-form="contact"]');g==null||g.addEventListener("submit",h=>{h.preventDefault(),Ya(g)});const A=r.querySelector('[data-form="task"]');if(A==null||A.addEventListener("submit",h=>{h.preventDefault(),La(A)}),A){const h=A.querySelector('select[name="instanceId"]');h==null||h.addEventListener("change",()=>{if(!J||!P)return;const R=Number(h.value);if(!Number.isFinite(R)||R<=0)return;const N=new FormData(A),V=String(N.get("due")??"").trim();P={...P,instanceId:R,parentUid:P.parentUid&&me.some(H=>H.uid===P.parentUid&&H.instanceId===R)?P.parentUid:null,summary:String(N.get("summary")??""),description:String(N.get("description")??""),status:String(N.get("status")??"NEEDS-ACTION"),due:V?new Date(V).toISOString():null,priority:Number(N.get("priority")??0),percent:Number(N.get("percent")??0)},m()})}const T=r.querySelector('[data-form="note"]');T==null||T.addEventListener("submit",h=>{h.preventDefault(),qa(T)});const w=r.querySelector('input[data-action="contact-search"]');w==null||w.addEventListener("input",()=>{Ie&&clearTimeout(Ie),Ie=setTimeout(()=>{Be=w.value,I!==null&&(async()=>{try{await Fe(I),m()}catch(h){v("error",h instanceof Error?h.message:"Search failed"),m()}})()},250)});const L=r.querySelector('input[data-action="task-search"]');L==null||L.addEventListener("input",()=>{Ie&&clearTimeout(Ie),Ie=setTimeout(()=>{pt=L.value,(async()=>{try{await Je(),m()}catch(h){v("error",h instanceof Error?h.message:"Search failed"),m()}})()},250)});const ae=r.querySelector('input[data-action="note-search"]');ae==null||ae.addEventListener("input",()=>{Ie&&clearTimeout(Ie),Ie=setTimeout(()=>{ft=ae.value,(async()=>{try{await st(),m()}catch(h){v("error",h instanceof Error?h.message:"Search failed"),m()}})()},250)}),Va(),Fa(),Ma()}async function Aa(e){var l,u;const t=Kt();if(t.length===0){v("error","No writable tasks selected"),m();return}const a=t.map(n=>({instanceId:n.instanceId,uri:n.uri}));if(e==="bulk-task-delete"){if(!confirm(`Delete ${t.length} task${t.length===1?"":"s"}? CalDAV clients will sync the removal.`))return;p=!0,O(),m();try{const n=await q.bulkTasks({op:"delete",items:a});se=[],pe&&t.some(s=>X(s.instanceId,s.uri)===pe)&&(pe=null,P=null,J=!1),await Je(),n.failed>0?v("error",`Deleted ${n.ok}, failed ${n.failed}${n.errors[0]?`: ${n.errors[0]}`:""}`):v("success",`Deleted ${n.ok} task${n.ok===1?"":"s"}`)}catch(n){v("error",n instanceof Error?n.message:"Bulk delete failed")}finally{p=!1,m()}return}let o={};if(e==="bulk-task-status"){const n=r.querySelector("#bulk-task-status"),s=((l=n==null?void 0:n.value)==null?void 0:l.trim())??"";if(!s){v("error","Choose a status to apply"),m();return}o={status:s}}else if(e==="bulk-task-due"){const n=ut.trim();if(!n){v("error","Choose a due date to apply"),m();return}const s=/^\d{4}-\d{2}-\d{2}$/.test(n)?new Date(n+"T00:00:00"):new Date((n.length===16,n));if(Number.isNaN(s.getTime())){v("error","Invalid due date"),m();return}o={due:s.toISOString()}}else if(e==="bulk-task-clear-due")o={due:null};else if(e==="bulk-task-percent"){const n=r.querySelector("#bulk-task-percent"),s=((u=n==null?void 0:n.value)==null?void 0:u.trim())??"";if(s===""){v("error","Enter a percent complete (0–100)"),m();return}const i=Number(s);if(!Number.isFinite(i)||i<0||i>100){v("error","Percent must be between 0 and 100"),m();return}o={percent:Math.round(i)}}p=!0,O(),m();try{const n=await q.bulkTasks({op:"update",items:a,fields:o});if(await Je(),P&&!J){const i=X(P.instanceId,P.uri),f=me.find(y=>X(y.instanceId,y.uri)===i);f&&(P={...f})}const s=e==="bulk-task-status"?"status":e==="bulk-task-due"||e==="bulk-task-clear-due"?"due date":"percent";n.failed>0?v("error",`Updated ${s} on ${n.ok}, failed ${n.failed}${n.errors[0]?`: ${n.errors[0]}`:""}`):v("success",`Updated ${s} on ${n.ok} task${n.ok===1?"":"s"}`)}catch(n){v("error",n instanceof Error?n.message:"Bulk update failed")}finally{p=!1,m()}}async function La(e){const t=new FormData(e),a=String(t.get("summary")??"").trim(),o=String(t.get("description")??"").trim(),l=String(t.get("status")??"NEEDS-ACTION"),u=String(t.get("due")??"").trim(),n=u?new Date(u).toISOString():null,s=Number(t.get("priority")??0),i=Number(t.get("percent")??0),f=String(t.get("parentUid")??"").trim(),y=f===""?null:f;p=!0,O(),m();try{if(J){const g=Number(t.get("instanceId"));if(!Number.isFinite(g)||g<=0)throw new Error("Select a calendar");const A=await q.createTask({instanceId:g,summary:a,description:o,status:l,due:n,priority:s,percent:i,parentUid:y});J=!1,pe=X(A.task.instanceId,A.task.uri),P=A.task,v("success",y?"Subtask created":"Task created")}else if(P){const g=await q.updateTask(P.instanceId,P.uri,{summary:a,description:o,status:l,due:n,priority:s,percent:i,parentUid:y});P=g.task,pe=X(g.task.instanceId,g.task.uri),v("success","Task saved")}await Je()}catch(g){v("error",g instanceof Error?g.message:"Save failed")}finally{p=!1,m()}}async function qa(e){const t=new FormData(e),a=String(t.get("summary")??"").trim(),o=String(t.get("description")??"").trim(),l=String(t.get("dtstart")??"").trim(),u=l?new Date(l).toISOString():null;p=!0,O(),m();try{if(le){const n=Number(t.get("instanceId"));if(!Number.isFinite(n)||n<=0)throw new Error("Select a calendar");const s=await q.createNote({instanceId:n,summary:a,description:o,dtstart:u});le=!1,Ne=X(s.note.instanceId,s.note.uri),z=s.note,v("success","Note created")}else if(z){const n=await q.updateNote(z.instanceId,z.uri,{summary:a,description:o,dtstart:u});z=n.note,Ne=X(n.note.instanceId,n.note.uri),v("success","Note saved")}await st()}catch(n){v("error",n instanceof Error?n.message:"Save failed")}finally{p=!1,m()}}function Ma(){const e=r.querySelector('input[data-action="contact-photo"]');e&&e.addEventListener("change",()=>{(async()=>{var a;const t=(a=e.files)==null?void 0:a[0];if(e.value="",!!t){if(t.size>2.5*1024*1024){v("error","Photo is too large (max ~2 MB)"),m();return}try{const o=await wa(t);he=o,ue=`data:${t.type||"image/jpeg"};base64,${o}`,we=!1,m()}catch(o){v("error",o instanceof Error?o.message:"Failed to read photo"),m()}}})()})}function Fa(){const e=r.querySelector('[data-form="create-cal"]');if(!e)return;const t=e.querySelector('input[name="holidays"]'),a=e.querySelector("#holidays-country-wrap"),o=e.querySelector('input[name="displayname"]'),l=e.querySelector('input[name="readOnly"]');if(!t||!a)return;const u=()=>{const n=t.checked;a.hidden=!n,o&&(o.required=!n,n&&!o.value.trim()?o.placeholder="Auto: Holidays (XX)":n||(o.placeholder="Work")),n&&l&&(l.checked=!0)};t.addEventListener("change",u),u()}async function Pa(e){const t=new FormData(e),a=String(t.get("username")??""),o=String(t.get("password")??"");p=!0,O(),m(),B.event("login.attempt",{username:a});try{const l=await q.login(a,o);d=l.user,St(l.ui),B.event("login.ok",{username:(d==null?void 0:d.username)??a}),await ge(),v("success","Signed in")}catch(l){B.warn("login.failed",l instanceof Error?l.message:l),v("error",l instanceof Error?l.message:"Login failed")}finally{p=!1,m()}}async function Ra(e){if(S===null)return;const t=new FormData(e),a=String(t.get("username")??""),o=String(t.get("access")??"read");W=!0,p=!0,O(),m();try{await q.share(S,a,o),await tt(S),v("success",`Shared with ${a}`)}catch(l){v("error",l instanceof Error?l.message:"Share failed")}finally{p=!1,m()}}function rt(e){if(!b)return;const t=new FormData(e),a=e.querySelector('input[name="allDay"]');b={...b,summary:String(t.get("summary")??b.summary),description:String(t.get("description")??b.description),location:String(t.get("location")??b.location),instanceId:Number(t.get("instanceId"))||b.instanceId,allDay:(a==null?void 0:a.checked)??b.allDay,start:String(t.get("start")??b.start??""),end:String(t.get("end")??b.end??"")||null,repeat:ot(t),hasRrule:!!String(t.get("repeatFreq")??"").trim()}}function ot(e){const t=String(e.get("repeatFreq")??"").trim().toUpperCase();if(!t)return{freq:"",interval:1,until:null,count:null,byDay:[],endMode:"never"};const a=Math.max(1,Math.min(99,Number(e.get("repeatInterval")??1)||1)),o=String(e.get("repeatEndMode")??"never"),l=o==="until"||o==="count"?o:"never";let u=null,n=null;if(l==="until"){const i=String(e.get("repeatUntil")??"").trim();u=i?i.slice(0,10):null}else if(l==="count"){const i=Number(e.get("repeatCount")??0);n=Number.isFinite(i)&&i>0?Math.min(999,Math.round(i)):10}const s=e.getAll("repeatByDay").map(i=>String(i).toUpperCase()).filter(Boolean);return{freq:t,interval:a,until:u,count:n,byDay:s,endMode:l}}async function Ua(e){if(!b||!b.canWrite)return;const t=new FormData(e),a=String(t.get("summary")??"").trim(),o=String(t.get("description")??"").trim(),l=String(t.get("location")??"").trim(),u=t.get("allDay")==="on",n=String(t.get("start")??"").trim(),s=String(t.get("end")??"").trim(),i=Number(t.get("instanceId"))||b.instanceId,f=ot(t);if(!a){v("error","Title is required"),m();return}if(!n){v("error","Start is required"),m();return}let y,g;if(u)y=n.slice(0,10),g=s?s.slice(0,10):y;else if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const L=Ct(n,s||null);y=new Date(L.start).toISOString(),g=L.end?new Date(L.end).toISOString():null}else y=new Date(n).toISOString(),g=s?new Date(s).toISOString():null;const A=b.instanceId,T=b.uri,w=ce;p=!0,O(),_=!0,m(),B.event(w?"event.create":"event.update",{instanceId:i,uri:w?null:T,allDay:u,summary:a});try{const L={summary:a,description:o,location:l,allDay:u,start:y,end:g,instanceId:i,repeat:f},ae=w?await q.createEvent(i,L):await q.updateEvent(A,T,L);(S===null||ae.event.instanceId!==S)&&(S=ae.event.instanceId),await Se(),_=!1,b=null,ce=!1,E=null,B.event(w?"event.created":"event.saved",{uri:ae.event.uri,instanceId:ae.event.instanceId}),v("success",w?"Event created":"Event saved")}catch(L){B.warn("event.save failed",L instanceof Error?L.message:L),v("error",L instanceof Error?L.message:"Save failed")}finally{p=!1,m()}}async function ja(e){if(S===null)return;const t=new FormData(e),a=String(t.get("displayname")??"").trim(),o=String(t.get("description")??""),l=String(t.get("color")??"").trim();p=!0,O(),m();try{const u=await q.updateCalendar(S,{displayname:a,description:o,color:l});W=!0,await ge(),S=u.calendar.id,await tt(S),await Se(),v("success","Calendar updated")}catch(u){v("error",u instanceof Error?u.message:"Update failed")}finally{p=!1,m()}}async function Ba(e){const t=new FormData(e),a=String(t.get("displayname")??"").trim(),o=String(t.get("description")??""),l=String(t.get("color")??"").trim(),u=t.get("holidays")==="on",n=String(t.get("holidayCountry")??"").trim(),s=t.get("readOnly")==="on";if(ie=!0,u&&!n){v("error","Select a country for the holidays calendar"),m();return}if(!u&&!a){v("error","Display name is required"),m();return}p=!0,O(),ke=null,m();try{const i=await q.createCalendar({displayname:a,description:o,color:l,holidays:u,holidayCountry:u?n:void 0,readOnly:s});S=i.calendar.id,ie=!1,await ge();let f=`Created “${i.calendar.displayname}”`;const y=i.holidayImport??i.calendar.holidayImport;y&&(f+=`. Holidays imported: ${wt(y)}.`,ke={ok:!0,message:wt(y)}),s&&(f+=" Calendar is read-only."),v("success",f)}catch(i){ie=!0,v("error",i instanceof Error?i.message:"Create failed")}finally{p=!1,m()}}async function _a(e){var o,l,u;const t=e.target.closest("[data-action]");if(!t)return;const a=t.dataset.action;if(a&&B.debug(`action:${a}`,{id:t.dataset.id,tab:t.dataset.tab,uri:t.dataset.uri}),a==="close-import-progress"){M&&(M.phase==="done"||M.phase==="error")&&Ht();return}if(a==="logout"){p=!0,B.event("logout");try{await q.logout()}catch{}d=null,Te(),M=null,x=[],ne=[],S=null,de=[],I=null,xe=[],G=null,k=null,Z=!1,oe=!1,ye=!1,ie=!1,O(),p=!1,m();return}if(a==="select-cal"){const n=Number(t.dataset.id);if(!Number.isFinite(n))return;S=n,ke=null,p=!0,O(),m();try{await Se()}catch(s){v("error",s instanceof Error?s.message:"Failed to load calendar")}finally{p=!1,m()}return}if(a==="edit-cal"){const n=Number(t.dataset.id);if(!Number.isFinite(n)||!x.find(i=>i.id===n&&i.canShare))return;S=n,W=!0,te=null,ke=null,p=!0,O(),m();try{await tt(n),await Se()}catch(i){v("error",i instanceof Error?i.message:"Failed to open calendar")}finally{p=!1,m()}return}if(a==="close-cal-modal"){W=!1,m();return}if(a==="open-create-cal-modal"){ie=!0,W=!1,te=null,O(),m();return}if(a==="close-create-cal-modal"){ie=!1,O(),m();return}if(a==="delete-cal"){const n=Number(t.dataset.id);if(!Number.isFinite(n)||!x.find(i=>i.id===n&&i.canShare))return;te=n,W=!1,O(),m();return}if(a==="cancel-delete-cal"){te=null,m();return}if(a==="confirm-delete-cal"){const n=Number(t.dataset.id),s=r.querySelector("#delete-cal-confirm");if(!Number.isFinite(n)||!(s!=null&&s.checked))return;p=!0,O(),m();try{if(await q.deleteCalendar(n),S===n&&(S=null),te=null,W=!1,ne=[],Ee=[],await ge(),S===null){const i=Ft();i&&(S=i.id,await Se())}v("success","Calendar deleted")}catch(i){v("error",i instanceof Error?i.message:"Delete failed")}finally{p=!1,m()}return}if(a==="month-today"){const n=new Date;$e={y:n.getFullYear(),m:n.getMonth()},Ze=null,p=!0,m();try{await Se()}finally{p=!1,m()}return}if(a==="month-prev"||a==="month-next"){const n=a==="month-prev"?-1:1,s=new Date($e.y,$e.m+n,1);$e={y:s.getFullYear(),m:s.getMonth()},Ze=null,p=!0,m();try{await Se()}finally{p=!1,m()}return}if(a==="open-event"){e.stopPropagation();const n=Number(t.dataset.instance),s=t.dataset.uri??"";if(!Number.isFinite(n)||!s)return;p=!0,O(),m();try{const i=await q.getEvent(n,s);b={...i.event,repeat:i.event.repeat??bt()},ce=!1,_=!0,E=null,W=!1,te=null}catch(i){v("error",i instanceof Error?i.message:"Failed to open event")}finally{p=!1,m()}return}if(a==="open-event-day"){e.stopPropagation();const n=t.dataset.day??"";Ze=Ze===n?null:n,m();return}if(a==="new-event-day"){const n=e.target;if((o=n==null?void 0:n.closest)!=null&&o.call(n,".month-event, .month-event-more"))return;const s=t.dataset.day??"";if(!/^\d{4}-\d{2}-\d{2}$/.test(s))return;if(S===null){v("error","Select a calendar first"),m();return}const i=x.find(f=>f.id===S);if(!i||i.readOnly||!(i.canShare||i.access==="readwrite")){v("error","This calendar is read-only"),m();return}ce=!0,b=ga(s,S),_=!0,E=null,W=!1,te=null,O(),m();return}if(a==="close-event-modal"){_=!1,b=null,ce=!1,E=null,O(),m();return}if(a==="dt-open"){const n=t.dataset.dtField||"";if(!n)return;const s=r.querySelector('[data-form="edit-event"]');if(s&&b&&rt(s),(E==null?void 0:E.field)===n)E=null;else{const i=t.dataset.dtDateOnly==="1",f=t.dataset.dtClear!=="0",y=t.dataset.dtName||n;let g=Tt(n);!g&&(n==="due"||n==="dtstart"||n==="bulk-due")&&(g=nt().start);const A=at(g||ee(new Date)),[T,w]=A.date.split("-").map(Number);E={field:n,viewY:T,viewM:(w||1)-1,dateOnly:i,allowClear:f,name:y}}m();return}if(a==="dt-month-prev"||a==="dt-month-next"){if(!E)return;const n=a==="dt-month-prev"?-1:1,s=new Date(E.viewY,E.viewM+n,1);E={...E,viewY:s.getFullYear(),viewM:s.getMonth()},m();return}if(a==="dt-pick-day"){if(!E)return;const n=E.field,s=t.dataset.day??"";if(!/^\d{4}-\d{2}-\d{2}$/.test(s))return;const i=r.querySelector('[data-form="edit-event"]');i&&b&&rt(i);const f=E.dateOnly;if(f)De(n,s),E=null;else{const y=Tt(n),g=at(y||nt(s).start).hm;De(n,`${s}T${g}`),E={...E,viewY:Number(s.slice(0,4)),viewM:Number(s.slice(5,7))-1}}if(n==="start"&&b&&!f&&b.end){const y=new Date(String(b.start)),g=new Date(String(b.end));!Number.isNaN(y.getTime())&&!Number.isNaN(g.getTime())&&g<=y&&De("end",Me(new Date(y.getTime()+3600*1e3)))}m();return}if(a==="dt-pick-time"){if(!E||E.dateOnly)return;const n=E.field,s=t.dataset.hm??"";if(!/^\d{2}:\d{2}$/.test(s))return;const i=r.querySelector('[data-form="edit-event"]');i&&b&&rt(i);const f=Tt(n)||nt().start,g=`${at(f).date}T${s}`;if(De(n,g),n==="start"&&b){b={...b,allDay:!1};const A=b.end?at(String(b.end)):null,T=new Date(g);(!A||new Date(`${A.date}T${A.hm}`)<=T)&&De("end",Me(new Date(T.getTime()+3600*1e3)))}E=null,m();return}if(a==="dt-today"){if(!E)return;const n=E.field,s=r.querySelector('[data-form="edit-event"]');s&&b&&rt(s);const i=ee(new Date);if(E.dateOnly)De(n,i);else{const f=nt(i);n==="start"?(De("start",f.start),b&&!b.end&&De("end",f.end)):n==="end"?De("end",f.end):De(n,f.start)}E=null,m();return}if(a==="dt-clear"){if(!E||!E.allowClear)return;const n=E.field,s=r.querySelector('[data-form="edit-event"]');s&&b&&rt(s),De(n,null),E=null,m();return}if(a==="event-allday-toggle"){if(!b)return;const n=r.querySelector('[data-form="edit-event"]'),s=t.checked;if(n){const i=new FormData(n),f=String(i.get("start")??b.start??""),y=String(i.get("end")??b.end??"")||null;let g=f,A=y;if(s){const T=la(f,y);g=T.start,A=T.end}else{const T=f.slice(0,10),w=(y||f).slice(0,10),L=Ct(T,w);g=L.start,A=L.end}b={...b,summary:String(i.get("summary")??b.summary),description:String(i.get("description")??b.description),location:String(i.get("location")??b.location),instanceId:Number(i.get("instanceId"))||b.instanceId,allDay:s,start:g,end:A,repeat:ot(i)}}else b={...b,allDay:s};E=null,m();return}if(a==="event-repeat-freq"||a==="event-repeat-end"){if(!b)return;const n=r.querySelector('[data-form="edit-event"]');if(!n)return;const s=new FormData(n),i=n.querySelector('input[name="allDay"]'),f=ot(s);b={...b,summary:String(s.get("summary")??b.summary),description:String(s.get("description")??b.description),location:String(s.get("location")??b.location),instanceId:Number(s.get("instanceId"))||b.instanceId,allDay:(i==null?void 0:i.checked)??b.allDay,start:String(s.get("start")??b.start??""),end:String(s.get("end")??b.end??"")||null,repeat:f,hasRrule:!!String(s.get("repeatFreq")??"").trim()},f.freq&&f.endMode==="until"&&(E==null?void 0:E.field)==="end"&&(E=null),m();return}if(a==="delete-event"){if(!b||!b.canWrite||ce||!confirm("Delete this event? CalDAV clients will sync the removal."))return;const n=b.instanceId,s=b.uri;p=!0,O(),m();try{await q.deleteEvent(n,s),_=!1,b=null,await Se(),v("success","Event deleted")}catch(i){v("error",i instanceof Error?i.message:"Delete failed")}finally{p=!1,m()}return}if(a==="info"){const n=t.dataset.info??"";Ka(n);return}if(a==="info-close"){Xt();return}if(a==="flash-close"){O(),m();return}if(a==="tab"){const n=t.dataset.tab;if(n==="calendars"||n==="contacts"||n==="tasks"||n==="notes"){C=n,B.event("tab",{tab:n}),n!=="calendars"&&(W=!1,te=null),n!=="contacts"&&(K=null),O(),p=!0,m();try{n==="contacts"&&I!==null?await Fe(I):n==="calendars"?await Se():n==="tasks"?await Je():n==="notes"&&await st()}catch(s){B.warn("tab load failed",s instanceof Error?s.message:s),v("error",s instanceof Error?s.message:"Failed to load")}finally{p=!1,m()}}return}if(a==="sort-task"||a==="sort-note"){const n=t.dataset.sort||"";if(!n)return;if(a==="sort-task"){qe===n?Oe=Oe==="asc"?"desc":"asc":(qe=n,Oe=n==="due"||n==="summary"?"asc":"desc"),p=!0,m();try{await Je()}catch(s){v("error",s instanceof Error?s.message:"Sort failed")}finally{p=!1,m()}}else{Ke===n?He=He==="asc"?"desc":"asc":(Ke=n,He="asc"),p=!0,m();try{await st()}catch(s){v("error",s instanceof Error?s.message:"Sort failed")}finally{p=!1,m()}}return}if(a==="select-task"){if(e.target.closest("[data-stop-row], .task-check"))return;const n=Number(t.dataset.instance),s=t.dataset.uri??"";if(!Number.isFinite(n)||!s)return;const i=me.find(f=>f.instanceId===n&&f.uri===s)??null;J=!1,pe=X(n,s),P=i?{...i}:null,O(),m();return}if(a==="task-check"){e.preventDefault(),e.stopPropagation();const n=Number(t.dataset.instance),s=t.dataset.uri??"";if(!Number.isFinite(n)||!s)return;const i=X(n,s),f=me.find(y=>X(y.instanceId,y.uri)===i);if(!f||!f.canWrite||f.readOnly)return;se.includes(i)?se=se.filter(y=>y!==i):se=[...se,i],m();return}if(a==="task-select-all"){e.preventDefault();const n=me.filter(i=>i.canWrite&&!i.readOnly);n.length>0&&n.every(i=>se.includes(X(i.instanceId,i.uri)))?se=[]:se=n.map(i=>X(i.instanceId,i.uri)),m();return}if(a==="bulk-task-clear"){se=[],m();return}if(a==="bulk-task-status"||a==="bulk-task-due"||a==="bulk-task-clear-due"||a==="bulk-task-percent"||a==="bulk-task-delete"){Aa(a);return}if(a==="select-note"){const n=Number(t.dataset.instance),s=t.dataset.uri??"";if(!Number.isFinite(n)||!s)return;const i=et.find(f=>f.instanceId===n&&f.uri===s)??null;le=!1,Ne=X(n,s),z=i?{...i}:null,O(),m();return}if(a==="new-task"){J=!0,pe=null,P={uri:"",instanceId:((l=_e[0])==null?void 0:l.id)??0,calendarId:0,calendarName:"",calendarUri:"",uid:"",parentUid:null,summary:"",description:"",status:"NEEDS-ACTION",due:null,priority:0,percent:0,completed:null,lastmodified:0,readOnly:!1,canWrite:!0},O(),m();return}if(a==="new-subtask"){if(!P||J||!P.uid||!P.canWrite)return;const n=P;J=!0,pe=null,P={uri:"",instanceId:n.instanceId,calendarId:n.calendarId,calendarName:n.calendarName,calendarUri:n.calendarUri,uid:"",parentUid:n.uid,summary:"",description:"",status:"NEEDS-ACTION",due:null,priority:0,percent:0,completed:null,lastmodified:0,readOnly:!1,canWrite:!0},O(),m();return}if(a==="new-note"){le=!0,Ne=null,z={uri:"",instanceId:((u=Ve[0])==null?void 0:u.id)??0,calendarId:0,calendarName:"",calendarUri:"",summary:"",description:"",dtstart:new Date().toISOString(),lastmodified:0,readOnly:!1,canWrite:!0},O(),m();return}if(a==="cancel-task"){J=!1,P=null,pe=null,m();return}if(a==="cancel-note"){le=!1,z=null,Ne=null,m();return}if(a==="delete-task"){if(!P||J||!confirm("Delete this task? CalDAV clients will sync the removal."))return;p=!0,O(),m();try{await q.deleteTask(P.instanceId,P.uri),pe=null,P=null,await Je(),v("success","Task deleted")}catch(n){v("error",n instanceof Error?n.message:"Delete failed")}finally{p=!1,m()}return}if(a==="delete-note"){if(!z||le||!confirm("Delete this note? CalDAV clients will sync the removal."))return;p=!0,O(),m();try{await q.deleteNote(z.instanceId,z.uri),Ne=null,z=null,await st(),v("success","Note deleted")}catch(n){v("error",n instanceof Error?n.message:"Delete failed")}finally{p=!1,m()}return}if(a==="select-ab"){const n=Number(t.dataset.id);if(!Number.isFinite(n))return;I=n,ye=!1,Le=null,G=null,k=null,Z=!1,oe=!1,Be="",xe=[],ue=null,he=null,we=!1,O(),p=!0,m();try{await Fe(n)}catch(s){v("error",s instanceof Error?s.message:"Failed to load contacts")}finally{p=!1,m()}return}if(a==="edit-ab"){e.stopPropagation();const n=Number(t.dataset.id);if(!Number.isFinite(n)||!de.find(f=>f.id===n))return;const i=I!==n;I=n,ye=!0,oe=!1,Le=null,O(),i&&(G=null,k=null,Z=!1,Be="",xe=[],ue=null,he=null,we=!1),p=!0,m();try{i&&await Fe(n)}catch(f){v("error",f instanceof Error?f.message:"Failed to open address book")}finally{p=!1,m()}return}if(a==="close-ab-modal"){ye=!1,m();return}if(a==="select-contact"){const n=t.dataset.uri??"";if(!n)return;O();try{await va(n)}catch(s){v("error",s instanceof Error?s.message:"Failed to load contact")}m();return}if(a==="new-contact"){if(I===null)return;$a(),O(),m();return}if(a==="cancel-contact"||a==="close-contact-modal"){Z=!1,oe=!1,k=null,G=null,ue=null,he=null,we=!1,E=null,O(),m();return}if(a==="add-email"||a==="add-phone"||a==="add-custom"){if(!k)return;yt(),Array.isArray(k.emails)||(k.emails=[""]),Array.isArray(k.phones)||(k.phones=[{type:"cell",value:""}]),Array.isArray(k.custom)||(k.custom=[]),a==="add-email"?k.emails.length<10&&k.emails.push(""):a==="add-phone"?k.phones.length<10&&k.phones.push({type:"other",value:""}):k.custom.length<30&&k.custom.push({label:"",value:""}),m();return}if(a==="remove-email"){if(!k)return;yt();const n=Number(t.dataset.idx);if(!Number.isFinite(n))return;const s=Array.isArray(k.emails)?k.emails:[""];k.emails=s.filter((i,f)=>f!==n),k.emails.length===0&&(k.emails=[""]),m();return}if(a==="remove-phone"){if(!k)return;yt();const n=Number(t.dataset.idx);if(!Number.isFinite(n))return;const s=Array.isArray(k.phones)?k.phones:[{type:"cell",value:""}];k.phones=s.filter((i,f)=>f!==n),k.phones.length===0&&(k.phones=[{type:"cell",value:""}]),m();return}if(a==="remove-custom"){if(!k)return;yt();const n=Number(t.dataset.idx);if(!Number.isFinite(n))return;k.custom=(Array.isArray(k.custom)?k.custom:[]).filter((s,i)=>i!==n),m();return}if(a==="remove-photo"){ue=null,he=null,we=!0,k&&(k.hasPhoto=!1),m();return}if(a==="delete-contact"){if(I===null||!G||!confirm("Delete this contact? CardDAV clients will sync the removal."))return;p=!0,O(),oe=!0,m();try{await q.deleteContact(I,G),G=null,k=null,Z=!1,oe=!1,E=null,ue=null,await ge(),v("success","Contact deleted")}catch(n){v("error",n instanceof Error?n.message:"Delete failed")}finally{p=!1,m()}return}if(a==="delete-ab"){e.stopPropagation();const n=Number(t.dataset.id??I);if(!Number.isFinite(n)||!de.find(i=>i.id===n))return;K=n,ye=!1,oe=!1,O(),m();return}if(a==="cancel-delete-ab"){K=null,m();return}if(a==="confirm-delete-ab"){const n=Number(t.dataset.id),s=r.querySelector("#delete-ab-confirm");if(!Number.isFinite(n)||!(s!=null&&s.checked))return;const i=de.find(y=>y.id===n);if(!i)return;const f=(i.cardCount??0)>0;p=!0,O(),m();try{await q.deleteAddressBook(n,f),I===n&&(I=null,xe=[],k=null,G=null,Z=!1),K=null,ye=!1,oe=!1,await ge(),I===null&&de.length>0&&(I=de[0].id,await Fe(I)),v("success","Address book deleted")}catch(y){v("error",y instanceof Error?y.message:"Delete failed")}finally{p=!1,m()}return}if(a==="export-ab"){if(I===null)return;ye=!0,p=!0,O(),m();try{const{blob:n,filename:s}=await q.exportAddressBook(I),i=URL.createObjectURL(n),f=document.createElement("a");f.href=i,f.download=s,f.click(),URL.revokeObjectURL(i),v("success",`Exported ${s}`)}catch(n){v("error",n instanceof Error?n.message:"Export failed")}finally{p=!1,m()}return}if(a==="export-contact"){if(I===null||!G||Z)return;oe=!0,p=!0,O(),m();try{const{blob:n,filename:s}=await q.exportContact(I,G),i=URL.createObjectURL(n),f=document.createElement("a");f.href=i,f.download=s,f.click(),URL.revokeObjectURL(i),v("success",`Exported ${s}`)}catch(n){v("error",n instanceof Error?n.message:"Export failed")}finally{p=!1,m()}return}if(a==="revoke"){const n=t.dataset.href??"";if(!n||S===null||!confirm("Revoke access for this user?"))return;W=!0,p=!0,O(),m();try{await q.revoke(S,n),await tt(S),v("success","Share revoked")}catch(s){v("error",s instanceof Error?s.message:"Revoke failed")}finally{p=!1,m()}return}if(a==="export-cal"){if(S===null)return;W=!0,p=!0,O(),m();try{const{blob:n,filename:s}=await q.exportCalendar(S),i=URL.createObjectURL(n),f=document.createElement("a");f.href=i,f.download=s,f.click(),URL.revokeObjectURL(i),v("success",`Exported ${s}`)}catch(n){v("error",n instanceof Error?n.message:"Export failed")}finally{p=!1,m()}}}function Va(){const e=r.querySelector('input[data-action="import-cal"]');e&&e.addEventListener("change",()=>{Ga(e)});const t=r.querySelector('input[data-action="import-create-cal"]');t&&t.addEventListener("change",()=>{Xa(t)});const a=r.querySelector('input[data-action="import-ab"]');a&&a.addEventListener("change",()=>{Ha(a)})}async function Ha(e){var o;if(I===null)return;const t=(o=e.files)==null?void 0:o[0];if(e.value="",!t)return;const a=I;ye=!0,p=!0,O(),Le=null,Te(),M={kind:"contacts",fileName:t.name,fileSizeLabel:_t(t.size),phase:"reading",readPercent:0,processPercent:null,processCurrent:0,processTotal:0,processImported:0,processUpdated:0,processSkipped:0,startedAt:Date.now(),elapsedSec:0,resultMessage:null,ok:null},Vt(),m();try{const l=await Jt(t,s=>{if(!M||M.phase!=="reading")return;M={...M,readPercent:s};const i=r.querySelector(".import-progress-bar"),f=r.querySelector("[data-import-status-line]");i&&s!==null&&(i.classList.remove("is-indeterminate"),i.style.width=`${s}%`),f&&s!==null&&(f.textContent=`Reading file… ${s}%`)});Ue("uploading",{readPercent:100}),Ue("processing",{processPercent:0}),B.event("import.contacts.start",{file:t.name,bytes:t.size,abId:a});const u=await q.importAddressBook(a,l,s=>{Wt(s)}),n=wt(u);Le={ok:!0,message:n},await ge(),I===a&&await Fe(a),Te(),Ue("done",{ok:!0,resultMessage:`${n} (from “${t.name}”)`}),v("success",`Import finished for “${t.name}”: ${n}.`)}catch(l){const u=l instanceof Error?l.message:"Import failed";Le={ok:!1,message:u},Te(),Ue("error",{ok:!1,resultMessage:u}),v("error",u)}finally{p=!1,m()}}function yt(){if(!k)return;const e=r.querySelector('[data-form="contact"]');if(!e)return;const t=new FormData(e);k.firstname=String(t.get("firstname")??""),k.lastname=String(t.get("lastname")??""),k.fullname=String(t.get("fullname")??""),k.org=String(t.get("org")??""),k.title=String(t.get("title")??""),k.url=String(t.get("url")??""),k.note=String(t.get("note")??"");const a=String(t.get("birthday")??"").trim();k.birthday=a&&/^\d{4}-\d{2}-\d{2}/.test(a)?a.slice(0,10):null,k.address={street:String(t.get("street")??""),city:String(t.get("city")??""),region:String(t.get("region")??""),postal:String(t.get("postal")??""),country:String(t.get("country")??"")};const o=[];let l=0;for(;t.has(`email_${l}`);)o.push(String(t.get(`email_${l}`)??"")),l++;o.length&&(k.emails=o);const u=[];for(l=0;t.has(`phone_value_${l}`);)u.push({type:String(t.get(`phone_type_${l}`)??"other"),value:String(t.get(`phone_value_${l}`)??"")}),l++;u.length&&(k.phones=u);const n=[];for(l=0;t.has(`custom_label_${l}`)||t.has(`custom_value_${l}`);)n.push({label:String(t.get(`custom_label_${l}`)??""),value:String(t.get(`custom_value_${l}`)??"")}),l++;k.custom=n}function Wa(e){const t=new FormData(e),a=[];let o=0;for(;t.has(`email_${o}`);){const s=String(t.get(`email_${o}`)??"").trim();s&&a.push(s),o++}const l=[];for(o=0;t.has(`phone_value_${o}`);){const s=String(t.get(`phone_value_${o}`)??"").trim();s&&l.push({type:String(t.get(`phone_type_${o}`)??"other"),value:s}),o++}const u=[];for(o=0;t.has(`custom_label_${o}`)||t.has(`custom_value_${o}`);){const s=String(t.get(`custom_label_${o}`)??"").trim(),i=String(t.get(`custom_value_${o}`)??"").trim();(s||i)&&u.push({label:s,value:i}),o++}const n={firstname:String(t.get("firstname")??"").trim(),lastname:String(t.get("lastname")??"").trim(),fullname:String(t.get("fullname")??"").trim(),org:String(t.get("org")??"").trim(),title:String(t.get("title")??"").trim(),emails:a,phones:l,address:{street:String(t.get("street")??"").trim(),city:String(t.get("city")??"").trim(),region:String(t.get("region")??"").trim(),postal:String(t.get("postal")??"").trim(),country:String(t.get("country")??"").trim()},url:String(t.get("url")??"").trim(),note:String(t.get("note")??"").trim(),birthday:(()=>{const s=String(t.get("birthday")??"").trim();return s&&/^\d{4}-\d{2}-\d{2}/.test(s)?s.slice(0,10):null})(),custom:u};return we?n.removePhoto=!0:he&&(n.photoBase64=he),n}async function Ya(e){if(I===null)return;const t=Wa(e);p=!0,O(),oe=!0,m();try{if(Z){const a=await q.createContact(I,t);Z=!1,G=a.contact.uri,k=null,oe=!1,ue=null,he=null,we=!1,E=null,v("success","Contact created")}else G&&(G=(await q.updateContact(I,G,t)).contact.uri,k=null,oe=!1,ue=null,he=null,we=!1,E=null,v("success","Contact saved"));try{await ge()}catch(a){if(console.error(a),I!==null)try{await Fe(I)}catch{}}}catch(a){v("error",a instanceof Error?a.message:"Save failed")}finally{p=!1,m()}}async function Ja(e){const t=new FormData(e),a=String(t.get("displayname")??"").trim(),o=String(t.get("description")??"").trim();if(a){p=!0,O(),m();try{const l=await q.createAddressBook({displayname:a,description:o});I=l.addressbook.id,G=null,k=null,Z=!1,Be="",await ge(),v("success",`Address book “${l.addressbook.displayname}” created`)}catch(l){v("error",l instanceof Error?l.message:"Create failed")}finally{p=!1,m()}}}async function za(e){if(I===null)return;const t=new FormData(e),a=String(t.get("displayname")??"").trim(),o=String(t.get("description")??"").trim();ye=!0,p=!0,O(),m();try{await q.updateAddressBook(I,{displayname:a,description:o}),await ge(),v("success","Address book updated")}catch(l){v("error",l instanceof Error?l.message:"Update failed")}finally{p=!1,m()}}function Ka(e){const t=rn[e];if(!t)return;const a=r.querySelector("#info-modal"),o=r.querySelector("#info-modal-title"),l=r.querySelector("#info-modal-body");if(!a||!o||!l)return;o.textContent=t.title,l.innerHTML=t.paragraphs.map(n=>`<p>${c(n)}</p>`).join(""),a.hidden=!1,document.body.classList.add("info-modal-open");const u=a.querySelector(".info-modal-close");u==null||u.focus()}function Xt(){const e=r.querySelector("#info-modal");e&&(e.hidden=!0,document.body.classList.remove("info-modal-open"))}async function Ga(e){var a;if(S===null)return;const t=(a=e.files)==null?void 0:a[0];e.value="",t&&(W=!0,await Qt(S,t,{keepEditModalOpen:!0}))}async function Xa(e){var f;const t=(f=e.files)==null?void 0:f[0];if(e.value="",!t)return;const a=r.querySelector('[data-form="create-cal"]'),o=a?new FormData(a):new FormData,l=o.get("holidays")==="on",u=o.get("readOnly")==="on";if(l){v("error","Turn off “Holidays calendar” to import a .ics file into a new calendar."),ie=!0,m();return}if(u){v("error","Turn off “Read-only” before importing — import cannot write to a read-only calendar."),ie=!0,m();return}let n=String(o.get("displayname")??"").trim();n||(n=t.name.replace(/\.ics$/i,"").trim()||"Imported calendar");const s=String(o.get("description")??""),i=String(o.get("color")??"").trim();p=!0,O(),ke=null,ie=!0,m();try{const y=await q.createCalendar({displayname:n,description:s,color:i,readOnly:!1});S=y.calendar.id,ie=!1,await ge(),v("success",`Created “${y.calendar.displayname}” — importing…`),await Qt(y.calendar.id,t,{keepEditModalOpen:!1,successPrefix:`Calendar “${y.calendar.displayname}” created. `})}catch(y){const g=y instanceof Error?y.message:"Create or import failed";ie=!0,v("error",g),p=!1,m()}}async function Qt(e,t,a={}){p=!0,O(),ke=null,Te(),M={kind:"calendar",fileName:t.name,fileSizeLabel:_t(t.size),phase:"reading",readPercent:0,processPercent:null,processCurrent:0,processTotal:0,processImported:0,processUpdated:0,processSkipped:0,startedAt:Date.now(),elapsedSec:0,resultMessage:null,ok:null},Vt(),m();try{const o=await Jt(t,n=>{if(!M||M.phase!=="reading")return;M={...M,readPercent:n};const s=r.querySelector(".import-progress-bar"),i=r.querySelector("[data-import-status-line]");s&&n!==null&&(s.classList.remove("is-indeterminate"),s.style.width=`${n}%`),i&&n!==null&&(i.textContent=`Reading file… ${n}%`)});Ue("uploading",{readPercent:100}),Ue("processing",{processPercent:0}),B.event("import.calendar.start",{file:t.name,bytes:t.size,calId:e});const l=await q.importCalendar(e,o,n=>{Wt(n)}),u=wt(l);ke={ok:!0,message:u},S===e&&await Se(),Te(),Ue("done",{ok:!0,resultMessage:`${u} (from “${t.name}”)`}),v("success",`${a.successPrefix||""}Import finished for “${t.name}”: ${u}.`)}catch(o){const l=o instanceof Error?o.message:"Import failed";ke={ok:!1,message:l},Te(),Ue("error",{ok:!1,resultMessage:l}),v("error",l)}finally{a.keepEditModalOpen&&(W=!0),p=!1,m()}}sa()}const na=document.getElementById("app");if(!na)throw new Error("#app missing");ln(na);
